...
define(function() {
  ...
  function SimPinDialog(dialog) {
    ...
    function unlockPin() {
      ...
      return unlockCardLock({ lockType: 'pin', pin: pin }).then(function() {
        clear();
      });
    }
    ...
    function verify() { // apply PIN|PUK
      switch (_action) {
        ...
        case 'unlock_pin':
          return unlockPin();
        case 'unlock_puk':
          return unlockPuk('puk');
        case 'unlock_puk2':
          return unlockPuk('puk2');

        // PIN lock
        case 'enable_lock':
          return enableLock(true);
        case 'disable_lock':
          return enableLock(false);
        case 'change_pin':
          return changePin('pin');

        // get PIN2 code (FDN contact list)
        case 'get_pin2':
          return updateFdnContact();

        // PIN2 lock (FDN)
        case 'enable_fdn':
          return enableFdn(true);
        case 'disable_fdn':
          return enableFdn(false);
        case 'change_pin2':
          return changePin('pin2');
      }
